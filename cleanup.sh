...
echo ""
echo "[2/8] Создание резервной копии..."
create_backup

# Шаг 3: Удаление PV файлов
remove_files "**/persistentvolumes/*.yaml" "Удаление PersistentVolumes"

# Шаг 3.5: Удаление системных RBAC файлов (они конфликтуют с существующими в Minikube)
echo ""
echo "🗑️  Удаление системных RBAC ресурсов (они уже есть в Minikube)"

for rbac_file in "$BACKUP_DIR"/cluster-scoped/clusterrolebindings.yaml "$BACKUP_DIR"/cluster-scoped/clusterroles.yaml; do
  if [ -f "$rbac_file" ]; then
    if [ "$DRY_RUN" = "true" ]; then
      echo "  [DRY RUN] Были бы удалены системные ресурсы из: $(basename $rbac_file)"
    else
      # Удаляем все системные ресурсы (system:*, kubeadm:*, kubelets:*, cluster-admin и т.д.)
      yq eval 'del(.items[] | select(.metadata.name | test("^(system:|kubeadm:|kubelets:|cluster-admin|kubernetes-dashboard)")))' -i "$rbac_file"
      echo "  ✓ Очищены системные ресурсы из: $(basename $rbac_file)"
    fi
  fi
done
...
# Шаг 6.5: Удаление CRD'ов с очень большими аннотациями (они не критичны)
echo ""
echo "⚙️  Удаление проблемных CRD'ов из файлов"

for crd_file in "$BACKUP_DIR"/cluster-scoped/customresourcedefinitions.yaml "$BACKUP_DIR"/cluster-scoped/crds.yaml; do
  if [ -f "$crd_file" ]; then
    if [ "$DRY_RUN" != "true" ]; then
      # Удаляем три CRD'а с слишком большими аннотациями
      yq eval 'del(.items[] | select(.metadata.name == "loggings.logging.banzaicloud.io" or .metadata.name == "prometheusagents.monitoring.coreos.com" or .metadata.name == "prometheuses.monitoring.coreos.com"))' -i "$crd_file"
      echo "  ✓ Удалены проблемные CRD'ы из: $crd_file"
    fi
  fi
done

if [ "$DRY_RUN" = "true" ]; then
  echo "  [DRY RUN] Были бы удалены CRD'ы: loggings, prometheusagents, prometheuses"
fi
