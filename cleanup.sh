...
apply_yq() {
  ...
  local count=$(find "$BACKUP_DIR" -type f -name "*.yaml" -path "$file_pattern" 2>/dev/null | wc -l || echo 0)
  
  if [ "$count" -gt 0 ]; then
    if [ "$DRY_RUN" = "true" ]; then
      echo "  [DRY RUN] Команда yq будет применена к $count файлам:"
      echo "  yq eval '$yq_command' -i"
      find "$BACKUP_DIR" -type f -name "*.yaml" -path "$file_pattern" 2>/dev/null | head -3
      [ "$count" -gt 3 ] && echo "  ... и еще $((count - 3)) файлам"
    else
      find "$BACKUP_DIR" -type f -name "*.yaml" -path "$file_pattern" -exec yq eval "$yq_command" -i {} \;
...
apply_yq "*/**/*.yaml" \
  'del(.metadata.uid, .metadata.resourceVersion, .metadata.generation, .metadata.creationTimestamp, .metadata.selfLink, .metadata.managedFields)' \
